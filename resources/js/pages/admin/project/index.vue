<template>
  <div>
    <h1 class="subheading grey--text">لیست پروژه ها</h1>
    <v-container>
      <v-data-table
        :headers="headers"
        :items="desserts"
      ...
      >
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <v-icon color="blue" dense class="mr-2" @click="viewProject(item)">
            mdi-eye
          </v-icon>
        ...
        <template v-slot:no-data>
          <v-btn color="primary" @click="getProjects"> موردی یافت نشد </v-btn>
        </template>
      </v-data-table>
    </v-container>
  </div>
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: "وضعیت پروژه", value: "status" },
      { text: "تصویر شاخص", value: "image" },
  ...
  methods: {
    ...
    viewProject(item) {
      this.$router.push(`/dashboard/project/view/${item.id}`);
    },
    editItem(item) {
