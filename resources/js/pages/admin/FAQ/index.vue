<template>
  <div>
    <h1 class="subheading grey--text">لیست سوالات متداول</h1>
    <v-container>
      <v-card>
        <v-card-title>
          <router-link class="a-style" to="/dashboard/FAQ/create">
        ...
        >
          ...
          <template v-slot:[`item.reply`]="{ item }">
            <div v-html="item.reply" class="mr-16"></div>
          </template>
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: "سوال", value: "question", align: "start", sortable: false },
      { text: "پاسخ", value: "reply", align: "center", sortable: false },
  ...
  methods: {
    ...
    editItem(item) {
      this.$router.push(`/dashboard/FAQ/edit/${item.id}`);
    },
    deleteItem(item) {
